...
(async () => {
  ...
  const PROSE = new Set(['P', 'H1', 'H2', 'H3', 'H4', 'H5', 'H6', 'BLOCKQUOTE']);
  const leafText = (el) => {
    const parts = [];
    (function walk(node) {
  ...
  const detectLists = (root) => {
    ...
    for (const parent of root.querySelectorAll('*')) {
      ...
      for (const items of groups.values()) {
        ...
        if (!items.every((x) => !PROSE.has(x.tagName) && x.querySelectorAll('*').length >= 2)) continue;

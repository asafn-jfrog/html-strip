 ...
 * copies it to the clipboard. Loads Readability + Turndown from a CDN on the fly.
 *
 ...
 *
(async()=>{const[{Readability},{default:TurndownService}]=await Promise.all([import('https://esm.sh/@mozilla/readability'),import('https://esm.sh/turndown')]);const a=new Readability(document.cloneNode(true)).parse();if(!a||!a.content)return alert('html-strip: no main content found on this page');const td=new TurndownService({headingStyle:'atx',codeBlockStyle:'fenced'});const h=[];if(a.title)h.push('# '+a.title);h.push('Source: '+location.href);if(a.byline)h.push('Author: '+a.byline);if(a.publishedTime)h.push('Date: '+a.publishedTime);if(a.siteName)h.push('Site: '+a.siteName);const md=h.join('\n')+'\n\n'+td.turndown(a.content).trim()+'\n';const ta=document.createElement('textarea');ta.value=md;document.body.appendChild(ta);ta.select();document.execCommand('copy');ta.remove();console.log('%chtml-strip: copied '+md.length+' chars to clipboard ✓','color:#0a0;font-weight:bold');})();
 *
 * ─────────────────────────────────────────────────────────────────────────
